...
exports.ApproveRequest = async (req, res, next) => {
  try {
    ...
    if (Object.keys(wasRejected).length) {
      ...
      //update the accepted request status to APPROVED
      await dbObject.UpdateRequestStatus(requestId, STATUS, HODApproval);
     

      return res.status(200).json({
        success: "Rejected reqest has successfully been approved",
      });
      const request = await dbObject.getRequestById(requestId);
      const { email, fullname } = request[0];
      //await SendRequestAcceptanceEmail(email, fullname);
   
    } else {
      const acceptedRequest = await dbObject.AcceptRequest(
        requestId,
        userId
      );
      if (acceptedRequest) {
