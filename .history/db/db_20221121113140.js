...
dbObject.GetSingleDepartmentRequestByRequestID = (departmentId, requestId) => {
  const sql =
    "SELECT requestId, fullname,deptname, requestDate,eventDate,eventTitle,eventType,eventvenuenumberOfPrint,numberOfRadio,numberOfTv,mediaType,eventBudget,requestStatus, isHODApproved from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON prrequests.deptId = departments.deptId where isHODApproved = 'NO' and department = ? and requestId = ?;";
  return new Promise((resolve, reject) => {
