...
dbObject.getAllDepartmentalRequests = (department) => {
  const sql =
    "SELECT requestId, fullname,deptname,eventDate,eventTitle,eventType,eventvenue,numberOfPrintPersonnel,mediaType,funding,eventBudget,requestStatus,isHODApproved from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON users.department = departments.deptId where department = ? order by requestDate asc;";
