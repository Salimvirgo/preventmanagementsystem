...
dbObject.GetSingleDepartmentRequestByRequestID = (departmentId, requestId) => {
  const sql =
    "SELECT requestId, fullname,deptname, requestDate,eventDate,eventTitle,eventType,eventvenue,numberOfPrint,numberOfRadio,numberOfTv,mediaType,eventBudget,requestStatus, is from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON prrequests.deptId = departments.deptId where department = ? and requestId = ?;";
  return new Promise((resolve, reject) => {
