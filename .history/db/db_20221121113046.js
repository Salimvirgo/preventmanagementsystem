...
dbObject.getAllRequests = () => {
  const sql =
    "SELECT requestId, fullname,deptname,eventDate,eventTitle,eventType,eventvenue,numberOfPrint,mediaType,eventBudget,requestStatus,isHODApproved from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON users.department = departments.deptId where isHODApproved = 'Yes' order by requestDate asc;";
