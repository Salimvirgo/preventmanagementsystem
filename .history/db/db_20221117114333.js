...
dbObject.getNewRequests = () => {
  const sql =
    "SELECT fullname,deptname, requestDate,eventDate,eventTitle,eventType,eventvenue,numberOfMediaPersonnel,mediaType,funding,eventBudget,requestStatus, isHODApproved from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON users.department = departments.deptId where date(requestDate) between date(date_sub(curdate(), interval 3 day)) and curdate() and isHODApproved = 'NO';";
