...
dbObject.getRejectedRequestByID = (requestId) => {
  const sql = "SELECT rr.rejectId, rr.requestId, usr.fullname, number from rejectedRequests as rr inner join prrequests as pr on rr.requestId *  from rejectedRequests where requestId = ?;";
  return new Promise((resolve, reject) => {
    connectionPool.query(sql, [requestId], (err, result) => {
      if (err) return reject(err);
