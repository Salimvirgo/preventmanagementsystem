...
dbObject.GetNewRequestsByDept = (department) => {
  const sql =
    "SELECT requestId, fullname,deptname, requestDate,eventDate,eventTitle,eventType,eventvenue,numberOfMediaPersonnel,mediaType,funding,eventBudget,requestStatus, isHODApproved from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON prrequests.deptId = departments.deptId where department = ?;";
  return new Promise((resolve, reject) => {
    connectionPool.query(sql, [department], (err, result) => {
      if (err) return reject(err);
