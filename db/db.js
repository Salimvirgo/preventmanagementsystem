...
dbObject.addNewRequest = (request) => {
  ...
  const sql =
    "INSERT INTO prrequests (userId,deptId, eventDate,eventTitle,eventType,eventvenue,numberOfMediaPersonnel,mediaType,eventBudget) VALUES (?,?,?,?,?,?,?,?,?);";
...
dbObject.getRequestById = (id) => {
  const sql =
    "SELECT requestId, fullname,deptname, eventDate,eventTitle,eventType,eventvenue,numberOfMediaPersonnel,mediaType,eventBudget,requestStatus,isHODApproved from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON users.department = departments.deptId where requestId = ?";
...
dbObject.UpdateRequestStatus = (id, status, HODApproval) => {
  const sql = "update prrequests set requestStatus = ?, isHODApproved = ? where requestId = ?";
  return new Promise((resolve, reject) => {
...
dbObject.GetNewRequestsByDept = (department) => {
  const sql =
    "SELECT requestId, fullname,deptname, requestDate,eventDate,eventTitle,eventType,eventvenue,numberOfMediaPersonnel,mediaType,eventBudget,requestStatus, isHODApproved from prrequests inner join users ON prrequests.userId = users.userId inner join departments ON prrequests.deptId = departments.deptId where department = ?;";
  return new Promise((resolve, reject) => {
    connectionPool.query(sql, [department], (err, result) => {
      if (err) return reject(err);
...
dbObject.GetUserRequests = (userId) => {
  const sql =
    "SELECT requestId,requestDate,eventDate,eventTitle,eventType,eventvenue,numberOfMediaPersonnel,mediaType,eventBudget,requestStatus,isHODApproved from vehicleRequests WHERE userId = ?";
  return new Promise((resolve, reject) => {
...
dbObject.getUsersSingleRequest = (requestId) => {
  const sql =
    "select AR.requestId,fullname as 'Requestor',users.email,deptname,date(requestDate), eventDate,eventTitle,eventType,eventvenue,numberOfMediaPersonnel,mediaType,eventBudget,requestStatus, isHODApproved,regNumber,drivers.driverId,driverName,drivers.phoneNumber,drivers.email from approvedRequests as AR inner join vehicleRequests on AR.requestId = vehicleRequests.requestId inner join vehicles on AR.vehicleAssigned = vehicles.vehicleId inner join users on vehicleRequests.userId = users.userId inner join departments on users.department = departments.deptId inner join drivers on AR.vehicleAssigned = drivGetUsersAndDepartmentsers.vehicleId where AR.requestId = ?";
...
dbObject.deleteRequestByID = (requestId) => {
  const sql = "delete from vehicleRequests where requestId = ?;";
  return new Promise((resolve, reject) => {
    connectionPool.query(sql, [requestId], (err, result) => {
      if (err) return reject(err);
...
dbObject.updateApprovedRequestVehicle = (requestId, vehicleId) => {
  ...
  return new Promise((resolve, reject) => {
    connectionPool.query(sql, [vehicleId, requestId], (err, result) => {
      if (err) return reject(err);
