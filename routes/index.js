...
router.get("/view-logs", ensureAuthenticated, async (req, res) => {
  ...
  const userRequests = requests.map((request) => {
    return {
      requestId: request.requestId,
      requestDate: moment(request.requestDate).format("MMM Do YYYY"),
      departureDate: request.departureDate,
      departureTime: request.departureTime,
      returnDate: request.returnDate,
      returnTime: request.returnTime,
...
router.get(
  ...
  async (req, res, next) => {
    ...
    if (requestId != null && requestId != "undefined") {
      ...
      if (r.length > 0) {
        const { requestStatus } = r[0];
        if (requestStatus === "REJECTED") {
          //get rejected request details
        } else if (requestStatus === "APPROVED") {
          //get approved request details
          const singleRequest = await dbObject
            .getUsersSingleRequest(requestId)
            .catch((error) => {
              next(error);
            });
    ...
    res.render("user-single", { userId, requestId });
  }
);
